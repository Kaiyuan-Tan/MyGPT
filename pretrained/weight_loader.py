...
def load_weights_into_gpt(gpt, params):
    ...
    for b in range(len(params["blocks"])):
        ...
        gpt.trf_blocks[b].ffd.layers[0].weight = assign(
            gpt.trf_blocks[b].ffd.layers[0].weight, 
            params["blocks"][b]["mlp"]["c_fc"]["w"].T)
        gpt.trf_blocks[b].ffd.layers[0].bias = assign(
            gpt.trf_blocks[b].ffd.layers[0].bias, 
            params["blocks"][b]["mlp"]["c_fc"]["b"])
        gpt.trf_blocks[b].ffd.layers[2].weight = assign(
            gpt.trf_blocks[b].ffd.layers[2].weight, 
            params["blocks"][b]["mlp"]["c_proj"]["w"].T)

        gpt.trf_blocks[b].ffd.layers[2].bias = assign(
            gpt.trf_blocks[b].ffd.layers[2].bias, 
